...
- **Incendiary Boost**: operator-funded priority emission stream. Operator escrows svZCHF/sUSDS one-sided into der Bodensee Pool → pool receives a 30-day supplementary AuMM stream pegged to the 85th efficiency percentile × (2 − R). Claims are subtracted from the **LP emission tranche** (after der Bodensee bootstrap skim in Months 1–10) *before* equal/CCB distribution — not a CCB score multiplier. See [Bootstrap](08_bootstrap.md).

- **Priority Skim**: the mechanism by which Incendiary Boost emissions are subtracted from the **LP emission tranche** *before* equal or CCB distribution. Total block emission never changes — Incendiary claims reduce the remainder available to all other pools, directly diluting their share. Bootstrapping new pools has a real cost borne by the entire emission economy, not free inflation.

- **der Bodensee Pool (LBP)**: autonomous reserve and sole destination for **protocol-captured** revenue — a two-token Liquidity Bootstrapping Pool (AuMM + svZCHF) with linear time-decay weights over 18 months. **Genesis seed:** 1 AuMM + 1 svZCHF. Genesis weights: 90/10. End weights: 48/52 (fixed permanently after 18 months). **Protocol-captured** fee revenue (50% of swap fees on **other** pools + 100% of ERC-4626 yield fees) flows one-sided into the svZCHF side. **Swap fee inside der Bodensee:** **0.75%**, **100%** retained **in pool** for der Bodensee LPs. **Months 1–10:** also receives **decaying one-sided AuMM bootstrap** (see [Protocol formulas — Bodensee bootstrap (F-0)](11_formulas.md)). **After Month 10:** no further AuMM via emission. Price discovery forced by time-decay and real revenue inflows — no oracle, no manual trigger. See [Tokenomics](04_tokenomics.md) and [Protocol formulas (F-11)](11_formulas.md).
...
- **Sandbox**: permissionless default state. Any pool can deploy without a gauge from block 0. Sandbox pools receive no CCB emissions and no Incendiary Boost, but rank in the Efficiency Tournament. Open access without an emission exploit vector.
...
- **LP Bonus**: 50% of swap fee revenue on **non–der Bodensee pools** distributed to LPs as an additional yield stream, proportional to pool liquidity share. **Trades inside der Bodensee** use a separate **0.75%** fee; those fees stay **in pool** for der Bodensee LPs.
